...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    if (inputShape0.GetShapeSize() == inputShape1.GetShapeSize()){
        ...
        uint32_t dim0 = inputShape0.GetDimNum();
        uint32_t dim1 = inputShape1.GetDimNum();
        uint32_t shape00 = 0;
        uint32_t shape01 = 0;
        uint32_t shape10 = 0;
        uint32_t shape11 = 0;
  ...
  auto dt1 = context->GetInputShape(1)->GetStorageShape().GetShapeSize();
  if (inputNum < dt1){
      inputNum = dt1;
  }
  ...
  uint32_t ubDataNum = 16; // 是根据具体的代码逻辑设定的，不是通过算出来的
  ...
  */
  ...
  auto dim = context->GetInputShape(0)->GetStorageShape().GetDimNum();
  auto dim1 = context->GetInputShape(1)->GetStorageShape().GetDimNum();
  ...
  tiling.set_dataTypeCp(dt1);
  tiling.set_size(dim1);
  tiling.set_dim(dim);
