...
BEGIN_TILING_DATA_DEF(IsCloseTilingData)
    ...
    TILING_DATA_FIELD_DEF(uint32_t, dataTypeCp);
    ...
    TILING_DATA_FIELD_DEF(uint32_t, axis);                    // 要广播的维度
    TILING_DATA_FIELD_DEF(uint32_t, who);                     // 谁要进行广播 0 代表第一个输入 1 代表第二个输入
    TILING_DATA_FIELD_DEF(uint32_t, isBroadcast);             // 是否进行广播 0 代表不进行广播 1 代表进行广播
    TILING_DATA_FIELD_DEF(float, rtol);                       // 相对误差
    TILING_DATA_FIELD_DEF(float, atol);                       // 绝对误差
    TILING_DATA_FIELD_DEF(bool, equal_nan);                  // 是否忽略 NaN
END_TILING_DATA_DEF;
